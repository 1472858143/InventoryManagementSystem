...
- 密码存储应采用带随机盐的自适应哈希算法，推荐 `Argon2id`，工程实现中也可使用 `BCrypt`
- 核心操作应具备可追溯性
...
- 推荐使用带随机盐的 `Argon2id` 哈希方案
- 若当前工程实现更偏向 Spring Security 默认生态，也可采用 `BCrypt`
...
- 密码哈希建议采用带随机盐的自适应哈希算法，例如 `Argon2id` 或 `BCrypt`
- 角色名称唯一
- 角色编码唯一
- 用户与角色组合唯一
- 商品编号唯一
...
- 登录校验时通过哈希校验函数比对原始密码与哈希摘要是否匹配
- 推荐使用 `Argon2id` 校验；若工程实现采用 `BCrypt`，则按对应编码器进行比对
...
- 用户新增时，后端接收原始密码后先进行哈希处理，再写入 `user.password`
- 用户重置密码时，同样只保存新的密码哈希摘要
- 用户查询接口不得返回密码字段内容
