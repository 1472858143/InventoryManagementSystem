...
| JDK | JDK 17 | 与 Spring Boot 生态兼容性较好 |
| 构建工具 | Maven | 用于后端依赖管理与打包 |
...
初稿中包含 `purchase` 模块，但当前项目书已经明确“采购与供应商管理”不属于本次毕业设计范围，因此该模块应从正式结构设计中移除。
...
#### 4.8.1 为什么移除 purchase

因为当前毕业设计范围已经明确不包含采购与供应商管理，因此 `purchase` 模块不应继续保留在正式项目结构中。
...
3. 安装 JDK 17、MySQL 8.0.44、Nginx
