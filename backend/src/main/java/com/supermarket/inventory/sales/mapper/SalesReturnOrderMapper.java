...
public interface SalesReturnOrderMapper {
    ...
    @Insert("""
        INSERT INTO sales_return_order (
            return_no, customer_id, source_order_id, total_quantity, total_amount,
            operator_id, status, reason
        ) VALUES (
            #{returnNo}, #{customerId}, #{sourceOrderId}, #{totalQuantity}, #{totalAmount},
            #{operatorId}, #{status}, #{reason}
        )
        """)
    @Options(useGeneratedKeys = true, keyProperty = "id")
    int insert(SalesReturnOrder order);

    @Select("""
        SELECT id AS id, return_no AS returnNo, customer_id AS customerId,
        ...
        FROM sales_return_order
        WHERE id = #{id}
        """)
    SalesReturnOrder findById(@Param("id") Long id);

    @Update("""
        UPDATE sales_return_order
        SET status = #{status}
        WHERE id = #{id}
        """)
    ...
    @Select("""
        ...
        """)
    List<SalesReturnOrder> findAll(@Param("keyword") String keyword,
                                   @Param("subjectId") Long subjectId,
                                   @Param("startDate") LocalDate startDate,
                                   @Param("endDate") LocalDate endDate,
